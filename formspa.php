...
$query = "SELECT id_spa,id_akun FROM spa,akun";
...
if (isset($_GET['id_spa'])) {
    //echo "Ini id, " . $_GET['id_spa'];
}
...
if(isset($_POST['submit'])) { 
    ...
    $sql = "INSERT INTO `reservasi_spa`(`id_spa`,`id_akun`,`nama_pemesan`, `tanggal_hari`, `jumlah_pengunjung`,`total_harga`,`metode_pembayaran`) 
    VALUES ('$ids','$ida','$nama', '$tanggal', '$jumlah','$total','$metode')";
    $result = mysqli_query($conn, $sql);

    if ($result) {
        echo "<script>alert('Selamat data anda berhasil disimpan')</script>";
        header('location:hasilspa.php');
    } else {
...
<html lang="id">
<head>
    <title>StunGo | Form Pemesanan Reservasi Spa</title>
...
<body>
    <h1 class="judul1">Form Pemesanan Reservasi Spa</h1>
    <div class="jangan">
        <form method="POST" action="">
            ...
            <div class="form-group">
                <div class="form-group">
                    <label for="bank">Jenis Layanan:</label>
                    <select name="kisaran" id="kisaran" required>
                        <option>- Pilih Harga Kisaran -</option>
                        <option value="50000">Ekonomis (Rp50.000)</option>
                        <option value="200000">Superior (Rp200.000)</option>
                        <option value="500000">Luxury (Rp500.000)</option>
                    </select>
                </div>
            </div>
            <div class="form-group">
